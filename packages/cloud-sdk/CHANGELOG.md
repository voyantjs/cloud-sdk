...
- 2bbb61d: Surface the standardized API error envelope on `VoyantApiError`.
...
- d8b5f70: Relicense from FSL-1.1-Apache-2.0 to Apache-2.0. The `license` field in the
  published package metadata now reports `Apache-2.0`, and the bundled `LICENSE`
  file carries the standard Apache License 2.0 text.
...
- 8fa7f4d: Add the Voyant Realtime surface: a `client.realtime` namespace (publish,
...
- 5543b28: Return ready-to-use signed HLS and DASH playback URLs from `videos.mintToken`.

## 0.8.0

### Minor Changes

- f53f324: Add per-call and envelope crypto methods to `client.vault`: `encrypt`,
